...
TOKEN_VARIABLE = config['telegram']['token_variable']
...
async def sendMessage(websocket, client, contextId, message):
	await websocket.send(getMessageRequest(client["clientId"], contextId, message))
	response = websocket.recv()
...
async def sendLocation(websocket, client, contextId, location):
	await websocket.send(getLocationRequest(client["clientId"], contextId, location))
	response = websocket.recv()
	return json.loads( await asyncio.wait_for(response, CLIENT_TIMEOUT))
